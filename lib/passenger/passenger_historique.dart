...
class _ReservedRidesPageState extends State<ReservedRidesPage> {
  ...
  Future<void> cancelReservation(String trajetId, int placesReservees) async {
    try {
      ...
      if (confirmation != null && confirmation) {
        ...
        DocumentSnapshot trajetReserveSnapshot = await FirebaseFirestore.instance.collection('trajetreserver').doc(trajetId).get();
        if (trajetReserveSnapshot.exists) {
          ...
          await FirebaseFirestore.instance.collection('trajets').doc(trajetOriginalId).update({
            'nombrePlaces': FieldValue.increment(placesReservees),
          });

          // Supprimer la réservation de la collection des trajets réservés
          await FirebaseFirestore.instance.collection('trajetreserver').doc(trajetId).delete();
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                title: Text('Erreur'),
                content: Text('La réservation que vous essayez d\'annuler n\'existe pas.'),
                actions: <Widget>[
                  TextButton(
                    onPressed: () {
                      Navigator.of(context).pop();
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Erreur'),
            content: Text('Une erreur est survenue lors de l\'annulation de la réservation. Veuillez réessayer plus tard.'),
            actions: <Widget>[
              TextButton(
                onPressed: () {
                  Navigator.of(context).pop();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Mes trajets réservés'),
      ),
            ...
            child: Card(
              ...
              child: ListTile(
                title: Column(
                  ...
                  children: [
                    Text('Depart: $depart', style: TextStyle(fontWeight: FontWeight.bold)),
                    ...
                    Text('Numero du conducteur: $numeroTelephone', style: TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
                trailing: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.red, // Couleur rouge pour le bouton annuler
                  ),
                  child: Text(
                    'Annuler',
